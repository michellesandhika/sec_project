...
import { app, firestore } from "../services/config";
...
function Authentication() {
    ...
    signInWithEmailAndPassword(auth, data.email, data.password)
      .then((userCredential) => {
        if (!userCredential.user.emailVerified) {
          ...
          setErrorMessage(
            "A new verification link has been sent to your email account. Please verify your email before logging in."
          );
      ...
      .catch((error) => {
        ...
        switch (code) {
          case "auth/wrong-password":
            setErrorMessage(
              "Wrong password. Try entering your password again."
            );
            break;

          case "auth/user-not-found":
            setErrorMessage(
              "You account information is not in our database. Please sign up first."
            );
            break;
  ...
  const signup = (data) => {
    ...
    if (!validatePassword(data.password)) {
      setError(true);
      setErrorMessage(
        "Your password must be at least 8 characters and contains an uppercase letter, lowercase letter, and numbers."
      );
      return;
    }
    ...
    createUserWithEmailAndPassword(auth, data.email, data.password)
      .then((userCredential) => {
        ...
        const docRef = setDoc(
          doc(firestore, "Users", userCredential.user.email),
          {
